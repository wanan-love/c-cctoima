...
3. **移动采集耗时**：加密通道无法直接批量调接口，须浏览器懒加载滚动
   （每页 5 条，套餐 414 系列 ≈ 15~25 分钟/类型），整轮约 40~90 分钟属预期。
4. **IMA 无删除/替换/建文件夹接口**（官方 1.1.9 实测）→ 版本化文件 + 文件名编码层级；
   旧版本文件保留在知识库中（官方"保留两者"模型）。
5. **风控**：电信（瑞数 WAF）与广电对高频访问敏感，采集节奏带随机抖动（真人模式），
   GitHub runner IP 每次不同 + WARP 出口，触发频控概率低；若长期 412 需检查 WARP。
